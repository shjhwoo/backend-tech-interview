...
순서 정리:
인터럽트가 가능한지, 인터럽트 요청신호를 보낸다
인터럽트 플래그가 활성화되어 있는지 플래그 레지스터에서 확인한다. 활성화되어 있다면 인터럽트 시작한다
이떄 기존 작업은 스택 메모리에 백업해둔다
인터럽트는 여러개 있으므로, 인터럽트 벡터를 통해서 인터럽스 서비스 루틴의 시작주소를 알아낸다. 인터럽트 서비스 루틴이라는 프로그램을 실행.(메모리에 올라가 있다.)
실행이 끝나면 스택 영역에 저장해둔 정보를 다시 불러와서 작업 재개한다.
